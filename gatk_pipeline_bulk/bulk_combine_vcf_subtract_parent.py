...
for group in sorted(group_samples_dict.keys()):

    GROUP_NAME = group
    print(f"==================================\nWorking on group {GROUP_NAME}\n==================================")
    
    samples = group_samples_dict[group]
    ...
    sbatch_2_last_part = max([int(fname.split('-part')[1]) for fname in os.listdir(f"scripts_{GROUP_NAME}") if fname.startswith('sbatch_2')])
    ...
    for chrom in chrom_pos_sample_data_dict:
        for pos in sorted(chrom_pos_sample_data_dict[chrom]):
            ...
            GTs = []
            for sample in ordered_samples:
                GT = chrom_pos_sample_data_dict[chrom][pos][sample][0] if sample in chrom_pos_sample_data_dict[chrom][pos] else '.'
